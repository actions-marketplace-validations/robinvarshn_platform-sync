...
validateArgs() {
  ...
  then
    echo >&2 "Error: Repository token is required."
    exit 1
  fi
  ...
  then
    echo >&2 "Error: Brandname is required"
    exit 1
  else 
...
workflowStallHandler() {
  echo >&2 "Syncing the Platform Changes..."
  echo >&2 "It might take 3-4 minutes"
  ...
  then
    echo >&2 "Platform Synced Successfully..."
    echo >&2 "Fetching The PR Link..."
    ...
    if [[ ! -z "$response" ]] && [[ $response != "null" ]]
      then
        echo >&2 "PR Link": $response
      else
        echo >&2 "NO PR Found"  
    fi 
    else
    echo >&2 "PR Link Not Fetched Due To Some Error"
    fi
  else
    # Workflow finished with error
    echo >&2 "Platform syncing failed due to some error"
    exit 1
  fi
}
...
entrypoint() {
  ...
  validateArgs
    ...
    if [[ ! -z "$workflowId" ]] && [[ $workflowId != "null" ]]
      ...
      echo >&2 "Platform syncing failed due to no workflow triggered"
      exit 1
    fi  
}
